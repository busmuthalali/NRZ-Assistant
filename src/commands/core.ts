...
case"leaderboard":{await i.deferReply();const metric=i.options.getString("metric",true)as LeaderboardMetric;const payload=await buildLeaderboard(i.guild!,metric);const msg=await i.editReply(payload);if(metric==="voice"){setInterval(async()=>{try{await msg.edit(await buildLeaderboard(i.guild!,metric));}catch(error){console.error("[Voice] leaderboard refresh failed",error);}},VOICE_LEADERBOARD_REFRESH_MS);}return;}
